...
function enviarEmail($nombre, $email, $asunto, $mensaje) {
	...
	if( !empty($_FILES['archivo1']['name']) || !empty($_FILES['archivo2']['name']) ) {	
		    ...
		    if(is_uploaded_file($_FILES['archivo1']['tmp_name'])) {
				 ...
				 $archivo1 = fopen($_FILES['archivo1']['tmp_name'], "rb");
				 $datos1 = fread( $archivo1, filesize($_FILES['archivo1']['tmp_name']) );
				 fclose($archivo1);
            ...
            if(is_uploaded_file($_FILES['archivo2']['tmp_name'])) {
                // Leemos el archivo obteniéndolo como una cadena de texto:
               $archivo2 = fopen($_FILES['archivo2']['tmp_name'], "rb");
               $datos2 = fread( $archivo2, filesize($_FILES['archivo2']['tmp_name']) );
               fclose($archivo2);
            ...
            if($datos1 != "") {
                $mensaje .= "--".$separador."\r\n";
                $mensaje .=	"Content-Type: ".$_FILES['archivo1']['type']."; name='".$_FILES['archivo1']['name']."'\r\n";
                $mensaje .= "Content-Transfer-Encoding: base64\r\n";
                $mensaje .= "Content-Disposition: attachment; filename='".$_FILES['archivo1']['name']."'\r\n\r\n";
                $mensaje .= $datos1."\r\n\r\n";
            }   
	...
	if($ok) {
        ...
        print "<p>Haz <a href='ej5.php'>click para volver al formulario</a></p>";
    } else {
        $html  = "<html>";
		$html .= "<head>";

		// Después de 5 segundos de mostrarse esta página web de error se redirigiría a la URL especificada.
		$html .= "<meta http-equiv='refresh' content='5;url=ej5.php'>";

		$html .= "</head>";
		$html .= "<body>";
        $html .= "<p style='color:red;'>El E-Mail NO ha sido enviado</p>";
        $html .= "<p>Haz <a href='ej5.php'>click para volver al formulario</a></p>";
